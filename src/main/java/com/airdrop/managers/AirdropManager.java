...
public class AirdropManager {
    ...
    // Запуск з рандомними координатами (використовує WorldBorder або ліміт з конфігу)
    public boolean startAirdrop(Rarity rarity, World world) {
        Location loc = getRandomLocation(world);
        if (loc == null) return false;
        return launchAirdrop(rarity, world, loc);
    }

    // Запуск з конкретними координатами
    public boolean startAirdropAt(Rarity rarity, World world, int x, int z) {
        int y = world.getHighestBlockYAt(x, z) + 1;
        Location loc = new Location(world, x, y, z);
        return launchAirdrop(rarity, world, loc);
    }

    private boolean launchAirdrop(Rarity rarity, World world, Location loc) {
    ...
    /**
     * Рандомна локація з урахуванням WorldBorder або ліміту з конфігу.
     * Пріоритет: WorldBorder > spawn-radius з конфігу.
     */
    private Location getRandomLocation(World world) {
        Random rand = new Random();

        // Беремо розмір WorldBorder
        WorldBorder border = world.getWorldBorder();
        double borderSize = border.getSize() / 2.0; // getSize() повертає діаметр

        // Беремо ліміт з конфігу
        int configRadius = plugin.getConfig().getInt("spawn-radius", 2000);

        // Використовуємо менше значення щоб не виходити за межі
        int radius = (int) Math.min(borderSize - 50, configRadius); // -50 буфер від краю бордеру
        ...
        // Рандомні координати відносно центру бордеру
    ...
    private void spawnFirework(Location loc, Rarity rarity) {
        ...
        FireworkEffect effect = FireworkEffect.builder()
                ...
                .withColor(color).withFade(Color.WHITE)
    ...
    public Map<UUID, AirdropEvent> getActiveAirdrops() { return activeAirdrops; }
        ...
        while (it.hasNext()) {
            ...
            if (world == null || event.getWorld().equals(world)) {
                ...
                String msg = event.getRarity().getColor() + "§l[Айрдроп] §cАйрдроп "
                        + event.getRarity().getDisplayName() + " §cзупинено адміністратором.";
                for (Player p : event.getWorld().getPlayers()) p.sendMessage(msg);
                count++;
            }
        }
        return count;
